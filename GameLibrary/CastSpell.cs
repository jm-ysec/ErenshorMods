...
public class CastSpell : MonoBehaviour
{
	...
	public bool isSimPlayer;

	private bool isPlayerAlways;
	...
	public ParticleSystem LearnSpell;

	public Spell MyAura;

	private SpellVessel CurrentVessel;
	...
	private void Start()
	{
		...
		if (base.transform.GetComponent<SimPlayer>() != null)
		{
			isSimPlayer = true;
		}
	...
	private void Update()
	{
		if (MyChar != null)
		{
			if ((MyChar.MyStats.Charmed && MyChar.Master != null && !MyChar.Master.isNPC && MyChar.Master.transform.name == "Player") || isPlayerAlways)
			{
				isPlayer = true;
			}
			else if (!isPlayerAlways)
			{
				isPlayer = false;
			}
		}
	...
	{
		if (CurrentlyCasting == null || (CurrentVessel != null && CurrentVessel.isProc))
		{
			return false;
		}
		return true;
	...
	public bool StartSpell(Spell _spell, Stats _target)
	{
		if ((isPlayer || isSimPlayer) && MyChar.MyStats.Invisible)
		{
			MyChar.MyStats.BreakEffectsOnAction();
		}
		if (_spell.GrantInvisibility && _target != null && _target.GetComponent<SimPlayer>() != null && !_target.GetComponent<SimPlayer>().InGroup)
		{
			UpdateSocialLog.LogAdd("This spell can only be cast on group members, yourself, or a willing target", "yellow");
			return false;
		}
		if (CurrentlyCasting == null && MyChar != null && MyChar.Alive && !MyChar.MyStats.Stunned && !MyChar.MyStats.Feared)
		{
			if ((isPlayer || isSimPlayer) && (_spell.Type == Spell.SpellType.Beneficial || _spell.Type == Spell.SpellType.Heal || _spell.Type == Spell.SpellType.Misc) && (_target == null || _spell.SelfOnly || (_target != MyChar.MyStats && !_target.PlayerOrSimPlayer)))
			{
				bool flag = true;
				Stats stats = _target?.Myself?.MyNPC?.CurrentAggroTarget?.MyStats;
				if (stats != null && !_spell.SelfOnly && stats.PlayerOrSimPlayer)
				{
					_target = stats;
					flag = false;
				}
				if (flag)
				{
					_target = MyChar.MyStats;
				}
			}
			if (MyChar.MyStats.GetCurrentMana() >= _spell.ManaCost)
			{
				if (_target != null && (_spell.SpellRange > Vector3.Distance(base.transform.position, _target.transform.position) || MyChar?.MyNPC?.ThisSim != null))
				{
					_ = isPlayer;
					MyAnim.SetTrigger("CastAtk");
					CurrentlyCasting = _spell;
					castTime = _spell.SpellChargeTime;
					if (_spell.ChargeVariations.Count <= 0)
					{
						if (_spell.ChargeSound != null)
						{
							MyChar.MyAudio.PlayOneShot(_spell.ChargeSound, MyChar.MyAudio.volume * GameData.SpellVol * GameData.MasterVol);
						}
					}
					else
					{
						MyChar.MyAudio.clip = _spell.ChargeVariations[Random.Range(0, _spell.ChargeVariations.Count)];
						MyChar.MyAudio.time = (MyChar.MyAudio.clip.length * 60f - _spell.SpellChargeTime) / 60f;
						MyChar.MyAudio.volume = GameData.SpellVol * 0.5f * GameData.MasterVol;
						MyChar.MyAudio.Play();
					}
					if (isPlayer)
					{
						UpdateSocialLog.CombatLogAdd("You begin casting " + _spell.SpellName, "lightblue");
					}
					GameObject gameObject = Object.Instantiate(GameData.EffectDB.ParticleFXVessel);
					CurrentVessel = gameObject.GetComponent<SpellVessel>();
					CurrentVessel.CreateSpellChargeEffect(CurrentlyCasting, base.transform, _target, this, castTime, _useMana: true, _resonate: false);
					gameObject.GetComponent<AudioSource>().volume *= GameData.SFXVol * GameData.MasterVol;
					return true;
				}
				if (!MyChar.isNPC)
				{
					UpdateSocialLog.CombatLogAdd("You're too far away to do that!", "lightblue");
					return false;
				}
			}
			else if (!MyChar.isNPC)
			{
				UpdateSocialLog.CombatLogAdd("You need more mana!", "lightblue");
				return false;
			}
		}
		else if (MyChar != null && !MyChar.isNPC)
		{
			if (!MyChar.MyStats.Stunned && !MyChar.MyStats.Feared)
			{
				UpdateSocialLog.CombatLogAdd("You are already casting a spell...", "lightblue");
			}
			else
			{
				UpdateSocialLog.CombatLogAdd("Can't cast while stunned...", "lightblue");
			}
			return false;
		}
		return false;
	...
	public bool StartSpell(Spell _spell, Stats _target, float modifiedCastTime)
	{
		if (CurrentlyCasting == null && MyChar.Alive)
		{
			if ((isPlayer || isSimPlayer) && MyChar.MyStats.Invisible)
			{
				MyChar.MyStats.BreakEffectsOnAction();
			...
			if ((isPlayer || isSimPlayer) && (_spell.Type == Spell.SpellType.Beneficial || _spell.Type == Spell.SpellType.Heal || (_spell.Type == Spell.SpellType.Misc && _spell.SpellRange <= 0f)) && (_target == null || (_target != MyChar.MyStats && !_target.PlayerOrSimPlayer)))
			{
				_target = MyChar.MyStats;
			}
			if (_spell.SpellRange > Vector3.Distance(base.transform.position, _target.transform.position) || _spell.SpellRange == 0f)
			{
				MyAnim.SetTrigger("CastAtk");
				CurrentlyCasting = _spell;
				castTime = modifiedCastTime;
				if (_spell.ChargeVariations.Count <= 0)
				{
					MyChar.MyAudio.PlayOneShot(_spell.ChargeSound, MyChar.MyAudio.volume * GameData.SpellVol * GameData.MasterVol);
				}
				...
				if (isPlayer && castTime > 1f)
				{
					UpdateSocialLog.CombatLogAdd("You begin casting " + _spell.SpellName, "lightblue");
				}
				CurrentVessel = Object.Instantiate(GameData.EffectDB.ParticleFXVessel).GetComponent<SpellVessel>();
				CurrentVessel.CreateSpellChargeEffect(CurrentlyCasting, base.transform, _target, this, castTime, _useMana: false, _resonate: false);
				return true;
			}
			if (!MyChar.isNPC)
			{
		...
		{
			UpdateSocialLog.CombatLogAdd("You are already casting a spell...");
			return false;
		}
		return false;
	}

	public bool StartSpell(Spell _spell, Stats _target, float modifiedCastTime, bool _resonate)
	{
		if (MyChar.Alive)
		{
			if ((isPlayer || isSimPlayer) && MyChar.MyStats.Invisible)
			...
			if (_spell.GrantInvisibility && _target != null && _target != GameData.PlayerStats && (!(_target.GetComponent<SimPlayer>() != null) || !_target.GetComponent<SimPlayer>().InGroup))
			{
				UpdateSocialLog.CombatLogAdd("This spell can only be cast on group members, yourself, or a willing target", "lightblue");
			}
			if ((isPlayer || isSimPlayer) && (_spell.Type == Spell.SpellType.Beneficial || _spell.Type == Spell.SpellType.Heal || _spell.Type == Spell.SpellType.Misc) && (_target == null || (_target != MyChar.MyStats && !_target.PlayerOrSimPlayer)))
			{
				_target = MyChar.MyStats;
			}
			MyAnim.SetTrigger("CastAtk");
			CurrentlyCasting = _spell;
			castTime = modifiedCastTime;
			if (_spell.ChargeVariations.Count <= 0 && !_resonate)
			{
				MyChar.MyAudio.PlayOneShot(_spell.ChargeSound, MyChar.MyAudio.volume * GameData.SpellVol * GameData.MasterVol);
			}
			else if (!_resonate)
			...
			if (!_resonate && isPlayer && castTime > 1f)
			{
				UpdateSocialLog.CombatLogAdd("You begin casting " + _spell.SpellName, "lightblue");
			}
			CurrentVessel = Object.Instantiate(GameData.EffectDB.ParticleFXVessel).GetComponent<SpellVessel>();
			CurrentVessel.CreateSpellChargeEffect(CurrentlyCasting, base.transform, _target, this, castTime, _useMana: false, _resonate);
			return true;
		}
		return false;
	}

	public bool StartSpell(Spell _spell, Stats _target, float modifiedCastTime, bool _resonate, float _scaleDmg)
	{
		if (MyChar.Alive)
		{
			if ((isPlayer || isSimPlayer) && MyChar.MyStats.Invisible)
			...
			CurrentVessel = Object.Instantiate(GameData.EffectDB.ParticleFXVessel).GetComponent<SpellVessel>();
			CurrentVessel.CreateSpellChargeEffect(CurrentlyCasting, base.transform, _target, this, castTime, _useMana: false, _resonate, _scaleDmg);
			return true;
		}
		return false;
	}

	public bool StartSpellFromProc(Spell _spell, Stats _target, float modifiedCastTime, bool _resonating = false, float _scaleDmg = 1f)
	{
		if (MyChar.Alive)
		{
			if ((isPlayer || isSimPlayer) && (_spell.Type == Spell.SpellType.Beneficial || _spell.Type == Spell.SpellType.Heal || _spell.Type == Spell.SpellType.Misc) && (_target == null || (_target != MyChar.MyStats && !_target.PlayerOrSimPlayer)))
			...
			if (_spell.SpellRange > Vector3.Distance(base.transform.position, _target.transform.position) || _spell.SpellRange == 0f)
			{
				...
				Object.Instantiate(GameData.EffectDB.ParticleFXVessel).GetComponent<SpellVessel>().CreateSpellProc(_spell, base.transform, _target, this, Mathf.Clamp(modifiedCastTime + 1f, 1f, 1f), _useMana: false, _resonating);
				return true;
			}
		}
		return false;
	...
	public void ForceSpellToEndEarly()
	{
		if (CurrentVessel != null)
		{
			CurrentVessel.CompleteSpellEarlyWithPenalty();
		}
	}
	...
	public void Reset()
	{
		if (MyAnim != null)
		{
			MyAnim.SetTrigger("CastAtkEnd");
		}
		CurrentlyCasting = null;
	}

	public bool StartSpellNoAnim(Spell _spell, Stats _target, float modifiedCastTime)
	{
		if (CurrentlyCasting == null && MyChar.Alive)
		{
			if (isPlayer && _spell.Type == Spell.SpellType.Beneficial && _target != MyChar.MyStats && _target.GetComponent<SimPlayer>() == null)
			{
				_target = MyChar.MyStats;
			}
			if (_spell.SpellRange > Vector3.Distance(base.transform.position, _target.transform.position) || _spell.SpellRange == 0f)
			{
				...
				Object.Instantiate(GameData.EffectDB.ParticleFXVessel).GetComponent<SpellVessel>().CreateSpellChargeEffect(CurrentlyCasting, base.transform, _target, this, castTime, _useMana: false, _resonate: false);
				return true;
			}
			if (!MyChar.isNPC)
			{
	...
	public void InterruptCast()
	{
		if (CurrentVessel != null)
		{
			...
			{
				UpdateSocialLog.LogAdd(base.transform.name + "'s casting has been interrupted!", "#FF9000");
			}
			CurrentVessel.EndSpellNoCD();
		}
	}

	public void CompleteCastEarly()
	{
		if (CurrentlyCasting != null && CurrentVessel != null && CurrentVessel.spell != null)
		{
			CurrentVessel.ResolveEarly();
		}
	}

	public void EndCastWithPenalty()
	{
		if (CurrentlyCasting != null && CurrentVessel != null && CurrentVessel.spell != null)
		{
			CurrentVessel.CompleteSpellEarlyWithPenalty();
	...
	public void EndCastWithScaling()
	{
		...
		if (MyChar.isNPC && Vector3.Distance(base.transform.position, GameData.PlayerControl.transform.position) < 15f)
		{
			UpdateSocialLog.LogAdd(base.transform.name + " controls their chant.", "lightblue");
		}
	}
	...
	public float GetCastProgress()
	{
		...
		return CurrentVessel.GetCastProgress();
	}
}
